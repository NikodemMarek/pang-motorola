...
import { Colors, GAME_SIZE, ImagePath, RENDERER_SIZE } from '../const'
...
export default class SideMenu extends Container {
    ...
    constructor(
        ...
        ;([
            ...
            new BitmapText(`${(info.points || 0).toString()} ${info.points == 1? 'point': 'points'}`, { fontName: 'buttonLabelFont', tint: Colors.MENU_BUTTON }),
            new BitmapText(`${(((info.lives || 0) > 0? info.lives: 0) || 0).toString()} ${info.lives == 1? 'live': 'lives'}`, { fontName: 'buttonLabelFont', tint: Colors.MENU_BUTTON }),
            new BitmapText([ 'Harpoon', 'Double Harpoon', 'Power Wire', 'Vulcan Missile' ][info.gun || 0], { fontName: 'buttonLabelFont', tint: Colors.MENU_BUTTON }),
            ...
            new BitmapText(`${Math.ceil(info.forceFieldTimeLeft || 0)?.toString()}s ${info.forceFields || 0} Shield`, { fontName: 'buttonLabelFont', tint: Colors.MENU_BUTTON }),
